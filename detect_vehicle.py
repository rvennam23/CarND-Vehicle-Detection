...
from lesson_function import get_hog_features,color_hist,convert_color

heatmaps = []
heatmap_sum = np.zeros((720, 1280)).astype(np.float64)

def find_cars(img, scale, ystart, ystop, pix_per_cell, cell_per_block, orient, spatial_size, hist_bins,X_scaler,svc):
...
def process_image(img):
    ...
    out_img, heat_map = find_cars(img, scale=scale, ystart=ystart, ystop=ystop,
                            ...
                            orient=orient, spatial_size=spatial_size, hist_bins=hist_bins,X_scaler=X_scaler,svc=svc)
    ...
    heatmap_avg_thresh = apply_threshold(heatmap_avg, 2.5)
